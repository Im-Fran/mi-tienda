...
import { sendCustomerMagicLink, verifyCustomerMagicLink, getCustomerOAuthUrl } from "@/api/endpoints/auth"
...
export function CustomerLoginPage() {
  ...
  const { session, setSession } = useCustomerAuthStore()

  const base = `/store/${storeSlug}`

  useEffect(() => {
    if (session) navigate(base, { replace: true })
  }, [session, navigate, base])

  const { mutate: sendLink, isPending: sending } = useMutation({
    mutationFn: (email: string) => sendCustomerMagicLink(email),
  })

  const { mutate: verifyLink } = useMutation({
    mutationFn: ({ token, email }: { token: string; email: string }) =>
      verifyCustomerMagicLink(token, email),
    onSuccess: (data) => {
      setSession({ token: data.token, customer: data.customer })
    ...
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription>Access your account or continue as guest</CardDescription>
        </CardHeader>
        <CardContent className="space-y-5">
          ...
          {sent ? (
            <div className="text-center py-3">
              ...
              <p className="text-sm text-muted-foreground mt-1">We sent you a magic link.</p>
              <Button variant="link" className="mt-2" onClick={() => setSent(false)}>Try a different email</Button>
            </div>
          ) : (
